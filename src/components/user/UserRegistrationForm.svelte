<script lang="ts">
  ...
  import { Button, Heading, Hr, Modal, P, Spinner } from "flowbite-svelte";
  ...
  onMount(() => {
    ...
    const applicationID = $siteConfig.application;
    if (!user.application || !applicationID) {
      return;
    }
    loading = true;
    api
      .getSubmission(user.application)
      .then((app) => {
        application = app.form_application;
        model = app.application_submission.values ?? {};
        loading = false;
      })
      .catch((e) => {
        console.error(e);
