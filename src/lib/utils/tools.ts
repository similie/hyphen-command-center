import type { SiteConfig, UUID } from "$lib/types";
...
export function formatMoney(value: number, config: SiteConfig) {
  ...
  return `${config.currencySymbol}${formattedValue}`;
}
...
export const siteMoneyFormatConfig = (
  ...
  const divisor = config.currencyDivisor || 100;
  const formattedValue = (value / divisor).toFixed(2);
  return `${config.currencySymbol}${formattedValue}`;
};

export const siteMoneyFormat = async (value: number): Promise<string> => {
  const config = await siteConfigInstance();
  return siteMoneyFormatConfig(value, config);
};
...
export const errorMessageWithCode = (message: string) => {
  ...
  if (errorCode.length > 1) {
    return { code: parseInt(errorCode[0]), message: errorCode[1] };
  }
  return { code: 500, message: message };
};
